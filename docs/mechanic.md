...
- 吃子后，玩家可以将被吃的棋子放置到右手边的位置；
- 玩家在吃子后存储的是白方的棋子，直接派遣其上场时会被当做白方处理；
- 允许玩家通过交易的方式将白色棋子兑换成想要的黑色棋子，只要棋子价值足够，它可以是不同的棋子。
	- 标准国际象棋中棋子价值估算参考数值：兵 1   马和象 3   车 5   后 9
...
- 格子之间的缝隙可以设置墙壁阻挡棋子：
	- 对于斜线行走，只需要在左右其中一侧设置墙壁，都会阻止其行走，这一个规则会防止象穿过直排墙壁；
	- 对于马，需要能够完全阻挡“走日字”的线路，并且墙自身的高低差足够大，才能够阻止马前行。
- 高低差机制：
	- 行走一步时高低差过大时会阻止其行走，包括马；
	- 车、象、后作为能够伸展到无限距离的单位，允许其逐步爬坡；
	- 马作为可越子单位，走一格的高低差限制比其他单位要宽松一些。
- 完全禁止行走，或者限制特定棋子的格子：
	- 例如水池。
