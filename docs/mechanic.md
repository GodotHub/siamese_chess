...
- 非棋盘状态的描述，例如棋子本身的走法，评分的计算，都需要使用“规则协议”机制来表达；
- 包含缩写映射对应的棋子、评价系统、预留额外信息的空间、每次落子时的行为等等；

## 非标准棋子

- 玩家可以通过导入自定义的GDScript文件来添加自定义棋子；
- 非标准棋子只会出现在部分关卡的初始位置，或者特定棋子的机制中，不能够凭空生成；
- 棋子需要在配置文件中标注其单字符缩写，不能够与现有棋子缩写重复；
- 缩写支持单个Unicode，意味着缩写可以是中文，也可以是Emoji；

## 筹码

- 特殊棋子，可以叠在常规棋子的下方，也可以单独放置在棋盘中；
- 筹码自身是来自西洋跳棋的棋子，不过背面没有皇冠；
- 作为资源，棋子本身可以视情况移动跳棋棋子，例如可以携带棋子过去；
- 西洋跳棋中的跳棋，将会被视作“棋子”而非“资源”对待。

## 棋盘切换

![image](chessboard_connection.svg)

- 简陋地说，达成某种条件会出现切换棋盘的情况；
- 切换棋盘的条件举例：
	- Cheshire走到门口位置；
	- 赢下或输掉对局；
	- 单击棋盘中的某个物品（不一定是棋子）；
...
- 预先走子具体细节有多种方案，如下：
	- 为了避免误操作，只能规划未来一回合的着法；
	- 以Lichess、Chess.com等国际象棋平台的逻辑为准；
...
- 双方的着法会影响局面评分，自然不用多说，但是着法本身产生的分数变化，才是我们需要讨论的；
- 同时，我们还需要照顾到自定义棋子，这些棋子的评分与常规的评分稍有不同，甚至还需要为了不同的任务而设计不同的评分规则；
- 特别注意，传统的分数计算也包含了棋子价值以外的因素，例如以下来自[ChessProgrammingWiki](https://www.chessprogramming.org/Evaluation#Where_to_Start)的公式：
	```
	f(p) = 200(K-K')
		+ 9(Q-Q')
		+ 5(R-R')
