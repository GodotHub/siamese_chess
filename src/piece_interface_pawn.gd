...
static func execute_move(state:Chess.ChessState, move:Chess.Move) -> void:
	...
	if move.extra:
		match move.extra:
			"Q":
				state.add_piece(move.position_name_to, Chess.create_piece(PieceInterfaceQueen, state.get_piece(move.position_name_from).group))
			"R":
				state.add_piece(move.position_name_to, Chess.create_piece(PieceInterfaceRook, state.get_piece(move.position_name_from).group))
			"N":
				state.add_piece(move.position_name_to, Chess.create_piece(PieceInterfaceKnight, state.get_piece(move.position_name_from).group))
			"B":
				state.add_piece(move.position_name_to, Chess.create_piece(PieceInterfaceBishop, state.get_piece(move.position_name_from).group))
