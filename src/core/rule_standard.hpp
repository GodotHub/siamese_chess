...
class RuleStandard : public Rule
{
	...
	public:
		...
		virtual int alphabeta(State *_state, int _alpha, int _beta, int _depth, int _group = 0, bool _can_null = true, godot::Dictionary _history_table = {}, TranspositionTable *_transposition_table = nullptr, godot::Callable _is_timeup = godot::Callable(), godot::Callable _debug_output = godot::Callable());
		virtual void search(State *_state, int _group, TranspositionTable *_transposition_table = nullptr, godot::Callable _is_timeup = godot::Callable(), int _max_depth = 1000, godot::Callable _debug_output = godot::Callable());
